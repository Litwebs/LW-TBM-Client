...
export default function OrderDetail() {
  ...
  if (!order) return <AccountLayout title="Order"><p className="muted">Order not found.</p></AccountLayout>;
    ...
    <AccountLayout title={`Order ${order.id}`} subtitle={`Placed ${new Date(order.date).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}`}>
      <div className="account-section" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
        <div>
          ...
          <span style={{ marginLeft: 12, fontSize: 13, color: "var(--muted)" }}>{order.deliveryStatus}</span>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn" onClick={reorder}>Reorder</button>
          <Link to="/account/orders" className="btn btn-outline">Back</Link>
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 24 }}>
        <div className="account-section" style={{ margin: 0 }}>
          <div className="account-section-head"><h2>Items</h2></div>
          {order.items.map((it) => (
            <div key={it.key} style={{ display: "flex", gap: 16, padding: "16px 0", borderBottom: "1px solid var(--border)" }}>
              <img src={it.product.image} style={{ width: 80, height: 80, objectFit: "cover" }} alt="" />
              <div style={{ flex: 1, minWidth: 0 }}>
                <Link to={`/products/${it.product.slug}`} style={{ fontSize: 13, letterSpacing: "0.12em", textTransform: "uppercase", display: "block", marginBottom: 6 }}>{it.product.title}</Link>
                <div className="muted" style={{ fontSize: 12 }}>
                  {Object.entries(it.variant || {}).map(([k, v]) => `${k}: ${v}`).join(" · ")}
                  {Object.keys(it.variant || {}).length ? " · " : ""}Qty {it.qty}
                </div>
              </div>
              <div style={{ fontWeight: 500 }}>£{(it.product.price * it.qty).toFixed(2)}</div>
            </div>
          ))}
          <dl className="kv-grid" style={{ marginTop: 20 }}>
            <dt>Subtotal</dt><dd>£{order.subtotal.toFixed(2)}</dd>
            <dt>Shipping</dt><dd>{order.shipping_fee ? `£${order.shipping_fee.toFixed(2)}` : "Free"}</dd>
            <dt>VAT</dt><dd>£{order.tax.toFixed(2)}</dd>
            <dt style={{ color: "var(--ink)", fontWeight: 600 }}>Total</dt><dd style={{ fontWeight: 600 }}>£{order.total.toFixed(2)}</dd>
          </dl>
        </div>

        <div>
          <div className="account-section" style={{ marginBottom: 24 }}>
            <div className="account-section-head"><h2>Delivery</h2></div>
            <ul className="timeline">
              {STAGES.map((s, i) => (
                <li key={s} className={i < idx ? "done" : i === idx ? "active" : ""}>
                  <div>
                    <div className="timeline-label">{s}</div>
                    {i === idx && order.eta && <div className="timeline-date">ETA {new Date(order.eta).toLocaleDateString("en-GB")}</div>}
                    {i === 5 && order.deliveredOn && <div className="timeline-date">{new Date(order.deliveredOn).toLocaleDateString("en-GB")}</div>}
                  </div>
                </li>
              ))}
            </ul>
            <dl className="kv-grid" style={{ marginTop: 16 }}>
              <dt>Carrier</dt><dd>{order.carrier}</dd>
              <dt>Tracking</dt><dd>{order.tracking || "—"}</dd>
            </dl>
          </div>
          <div className="account-section" style={{ marginBottom: 24 }}>
            <div className="account-section-head"><h2>Shipping</h2></div>
          ...
          <div className="account-section" style={{ margin: 0 }}>
            <div className="account-section-head"><h2>Payment</h2></div>
            <div className="card-brand">{order.paymentMethod.brand}</div>
            <div className="card-num">•••• {order.paymentMethod.last4}</div>
            <div className="card-meta"><span>Exp {order.paymentMethod.exp}</span><span>Charged £{order.total.toFixed(2)}</span></div>
          </div>
        </div>
      </div>
    </AccountLayout>
