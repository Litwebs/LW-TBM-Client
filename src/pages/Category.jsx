  ...
  const jsonLd = [
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Categories", item: SITE_BASE + "/collections/all-panels" },
  ...
  return (
    <div className="container">
      ...
      <nav className="breadcrumbs" aria-label="Breadcrumb">
        <Link to="/">Home</Link> &nbsp;/&nbsp;{" "}
        <Link to="/collections/all-panels">Shop</Link> &nbsp;/&nbsp; <span>{name}</span>
      </nav>
      <header className="shop-header">
        <h1>{name}</h1>
        <p className="intro-text" style={{ marginTop: 12 }}>
          Discover our {String(name).toLowerCase()} collection — designed in Britain,
          engineered for modern interiors. Each piece is selected for craftsmanship,
      ...
      {loading && items.length === 0 ? (
        ...
        <p className="muted" style={{ textAlign: "center", padding: 60 }}>
          No products available in this category yet. <Link to="/contact">Contact us</Link> for a bespoke enquiry.
        </p>
      ) : (
      ...
      <section style={{ marginTop: 64 }}>
        <h2 className="section-title" style={{ marginBottom: 24 }}>Explore more categories</h2>
        <div className="product-grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))" }}>
          {categories
            .filter((c) => c.slug !== slug)
            .slice(0, 8)
            .map((c) => (
              <Link key={c.slug} to={`/categories/${c.slug}`} className="link" style={{ padding: 12, border: "1px solid #eee", borderRadius: 4, textAlign: "center" }}>
                {c.name}
              </Link>
            ))}
        </div>
